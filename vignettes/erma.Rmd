...
```{r bind}
ermaset@rowRanges = uil33  # rowRanges<- is broken 5/16
ermaset
```

Now query the files for cell-specific states in this interval.
While GenomicFiles reduce operations are failing, let's skip them.
...
csstates = bplapply( files(ermaset), function(x) {
    imp = import(x, which=uil33)
    imp$rgb = erma:::rgbByState(imp$name)
    imp
})
tys = cellTypes(ermaset)
csstates = bplapply( 1:length(csstates), function(x) {
   csstates[[x]]$celltype=tys[x]
   csstates[[x]]
})
```
